...
function cargarTablaPersonal() {
  fetch('obtener_personal_general.php')
    .then(res => res.json())
    .then(data => {
...
function mostrarModalEdicion(cedula, idHorario) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      window.location.href = `editar_horario_general.php?cedula=${encodeURIComponent(cedula)}&id=${encodeURIComponent(idHorario)}`;
    }
  });
}
...
function mostrarVistaPrevia(cedula) {
  Swal.fire({
    ...
    window.location.href = `vista_horarios_general.php?cedula=${encodeURIComponent(cedula)}`;
  });
}
...
<form method="POST" action="guardar_horarios_general.php">

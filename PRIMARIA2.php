...
function cargarTablaPersonal() {
  fetch('obtener_personal_primaria.php')
    .then(res => res.json())
    .then(data => {
...
function mostrarModalEdicion(cedula, idHorario) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      window.location.href = `editar_horario_primaria.php?cedula=${encodeURIComponent(cedula)}&id=${encodeURIComponent(idHorario)}`;
    }
  });
}
...
function mostrarVistaPrevia(cedula) {
  Swal.fire({
    ...
    window.location.href = `vista_horarios_primaria.php?cedula=${encodeURIComponent(cedula)}`;
  });
}
...
<form method="POST" action="guardar_horarios_primaria.php">

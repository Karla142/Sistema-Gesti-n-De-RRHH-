...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->Cell(0, 10, 'Tabla Personal', 0, 1, 'C');
    }
    ...
    function AddTableHeader()
    {
        ...
        $this->Cell(30, 10, 'Cédula', 1, 0, 'C', true);
        $this->Cell(40, 10, 'Correo', 1, 0, 'C', true);
        $this->Cell(30, 10, 'Fecha Nac.', 1, 0, 'C', true);
        $this->Cell(30, 10, 'Fecha Ingreso', 1, 0, 'C', true);
        $this->Cell(30, 10, 'Cargo', 1, 0, 'C', true);
        $this->Cell(30, 10, 'Acciones', 1, 0, 'C', true);
    }
    ...
    function AddTableRow($persona)
    {
        ...
        $this->Cell(30, 10, $persona['nombre_personal'], 1, 0, 'C');
        $this->Cell(30, 10, $persona['apellido_personal'], 1, 0, 'C');
        $this->Cell(30, 10, $persona['cedula_personal'], 1, 0, 'C');
        $this->Cell(40, 10, $persona['correo_personal'], 1, 0, 'C');
        $this->Cell(30, 10, $persona['nacimiento_personal'], 1, 0, 'C');
        $this->Cell(30, 10, $persona['ingreso_personal'], 1, 0, 'C');
        $this->Cell(30, 10, $persona['cargo_personal'], 1, 0, 'C');
        $this->Cell(30, 10, "<a href='generar_pdf.php?id={$persona['id_personal']}'><img src='descargar.png' alt='Descargar' width='25' height='22'></a>", 1, 0, 'C');
    }
}

if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $sql = "SELECT nombre_personal, apellido_personal, cedula_personal,correo_personal, nacimiento_personal, ingreso_personal, cargo_personal FROM persona WHERE id_personal = $id";
    $result = $conn->query($sql);
    $persona = $result->fetch_assoc();

    $pdf = new PDF('L', 'mm', 'A4'); // Cambio a orientación horizontal (Landscape)
    $pdf->AddPage();
    $pdf->AddTableHeader();
    $pdf->AddTableRow($persona);

    $pdf->Output("D", "{$persona['nombre_personal']}.pdf");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['all'])) {
    ...
    $pdf->Output("D", "todos_los_datos.pdf");
    exit;
}
?>
...
<div class="logo-container">
    <a href="tabla_reportes_personal.php">
...
<div id="userProfileModal" class="modal">
    ...
    <div class="letra">
<h1>Gestión De Reportes Personal</h1>
</div>
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            echo "<tr>
                    ...
                    <td>
                    <a href='generar.php?id_personal=" . $row["id_personal"] . "' class='hover-text'>
...
<style>
  ...
  .hover-text::after {
    content: 'Haz clic para descargar';
    position: absolute;
    bottom: 95; /* Ajusta según necesites */
    left: 82%;
